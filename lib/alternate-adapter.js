...
firepad.AlternateAdapter = (function (global) {
  ...
  function AlternateAdapter (ref, userId, userColor) {
    ...
    this.zombie_ = false;
    this.socket = io('http://localhost:3000/646465465');
  ...
  AlternateAdapter.prototype.sendOperation = function (operation, callback) {
    console.log('ALT SEND OPERATION', operation);
    var self = this;
    ...
    console.log('DOCTARGETLENGTH', this.document_.targetLength, operation.baseLength);
    // FIXME: disabled santity check
    ...
    function doTransaction(revisionId, revisionData) {
      var revision = {};
      revision[revisionId] = {revision: revisionData};
      console.log('ALT SENDING REVISION');
  ...
  AlternateAdapter.prototype.registerCallbacks = function(callbacks) {
    for (var eventType in callbacks) {
      console.log('REGISTERING EVENTTYPE', eventType);
  ...
  AlternateAdapter.prototype.monitorHistoryStartingAt_ = function(revision) {
    ...
    var self = this;
    console.log('IN MONITORY HISTORY STARTING AT');
    self.socket.on('revision', function(e) {
      console.log('RECEIVED REVISION');
      for(key in e) {
        self.pendingReceivedRevisions_[key] = e[key];
      }
      if(self.ready_) {
    ...
    //     console.log('MONITORHISTORY', revisionSnapshot);
  ...
  AlternateAdapter.prototype.handlePendingReceivedRevisions_ = function() {
    ...
    while (pending[revisionId] != null) {
      console.log('PENDINGREVISION', revisionId, pending[revisionId]);
      this.revision_++;

      var revision = this.parseRevision_(pending[revisionId].revision);
      if (!revision) {
        // If a misbehaved client adds a bad operation, just ignore it.
        utils.log('Invalid operation.', this.ref_.toString(), revisionId, pending[revisionId]);
      } else {
  ...
  AlternateAdapter.prototype.parseRevision_ = function(data) {
    ...
    try {
      ...
      console.log('TEXTOPERATIONFAILED');
      return null;
    }
    // FIXME: Disabled this check
