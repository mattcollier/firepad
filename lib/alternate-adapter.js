...
firepad.AlternateAdapter = (function (global) {
  ...
  function AlternateAdapter (ref, userId, userColor) {
    this.ref_ = ref;
    this.headless = this.ref_.headless || false;
    this.ready_ = false;
    this.firebaseCallbacks_ = [];
    this.zombie_ = false;

    // headless = true indicates that we're operating under headless controller
    // in node and not in browser
    if(!this.ref_.headless) {
      // send a post to the server with documentID
      // which sets up web sockets on the server
      // headless operation does not require web sockets
      this.request = new XMLHttpRequest();
      // FIXME: make this request async
      this.request.open(
        'POST', this.ref_.server + '/document/' + this.ref_.document, false);
      this.request.setRequestHeader(
        'Content-Type', 'application/json;charset=UTF-8');
      this.request.send(null);
      if(!(this.request.status === 201 || this.request.status === 409)) {
        throw new Error(
          'The server could not be contacted:' + this.ref_.server);
      }
    }
  ...
  AlternateAdapter.prototype.monitorHistoryStartingAt_ = function(revision) {
    ...
    if(!self.ref_.headless) {
      ...
      setTimeout(function() {
        ...
        if(self.request.status === 200) {
          _.assign(
            self.pendingReceivedRevisions_,
    ...
    if(self.ref_.headless) {
      self.ref_.request(
        self.ref_.server + '/document/' + self.ref_.document + '/history',
        function(err, res, body) {
          if(!err && res.statusCode === 200) {
            self.ref_.lodash.assign(self.pendingReceivedRevisions_, body);
            self.handleInitialRevisions_();
          }
        });
    }
